...
int main(void){
  ...
  while(1){          // Follows the nine steps list above		
		// a) Ready signal goes high, green LED goes on
		SetReady();
...
void SetVT(void){
	VT = GPIO_PORTF_DATA_R;
	VT |= 0x02;
	GPIO_PORTF_DATA_R = VT;
}
...
void ClearVT(void){
	VT = GPIO_PORTF_DATA_R;
	VT &= ~0x02; //clear bit 2 which is PF1
	GPIO_PORTF_DATA_R = VT;
}
...
// Notes:   friendly means it does not affect other bits in the port
void SetReady(void){
	Ready = GPIO_PORTF_DATA_R;
	Ready |= 0x08; 
	GPIO_PORTF_DATA_R = Ready;
}
...
void ClearReady(void){
	Ready = GPIO_PORTF_DATA_R;
	Ready &= ~0x08; //clear bit 2 which is PF1
	GPIO_PORTF_DATA_R = Ready;
}
